...
# v0.21.82 — 设置新增工具调用区显示控制（始终折叠 / 隐藏）

> 用户反馈：会话窗口中工具调用记录（"已调用 N 个工具"）一直刷屏，不好看——需要设置控制默认折叠或隐藏。

## 实现（ui/index.html）

设置页新增两个开关（localStorage 持久化，刷新生效）：

1. **工具调用区始终折叠**（默认开）：对话中工具调用记录**保持折叠**（不随工具执行自动展开），点「展开 ▾」可手动查看——回复窗口不再被工具记录刷屏
2. **隐藏工具调用区**（默认关）：回复中**完全不显示**工具调用记录（含历史消息），界面更清爽——不影响工具实际执行，AI 照常调用

生效范围：新对话初始工具区（隐藏时不渲染）、流式执行中（始终折叠时不自动展开 / 隐藏时保持隐藏）、历史消息渲染（隐藏时跳过）。

## 验证

- 逻辑单测 4/4 PASS（默认运行时展开 / 始终折叠不展开 / 隐藏不显示 / 无工具不显示）；index.html 脚本语法通过
- 102/249 已热更；WebDAV 已同步（v0.21.82.fpk）
