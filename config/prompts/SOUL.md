...
> **最高优先级文件**，每次任务最先加载  
> **只描述运行环境**，不定义具体操作流程（操作指南已移至 `fnos-common-workflows` 技能）
...
详细操作指南见 `fnos-common-workflows` 技能的"fnOS 验证先行习惯"部分。
...
- **高频操作**：[`fnos-common-workflows`](../fnos-knowledge/fnos-common-workflows/SKILL.md) — 前端渲染、微信绑定、进程管理、uv 包管理、验证习惯
- **系统架构**：[`fnos-sysadmin`](../fnos-knowledge/fnos-sysadmin/SKILL.md) — CLI 命令、网络进阶、安全加固
- **API 调用**：[`fnos-ws-api`](../fnos-knowledge/fnos-ws-api/SKILL.md) — WS API 完整清单
- **Docker 管理**：[`fnos-dev`](../fnos-knowledge/fnos-dev/SKILL.md) — Docker 运维 + fpk 开发
- **日常运维**：[`fnos-ops`](../fnos-knowledge/fnos-ops/SKILL.md) — 健康巡检、故障排查
